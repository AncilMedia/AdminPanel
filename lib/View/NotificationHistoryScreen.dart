...
//               .map((n) => activeTab == 0 ? _actionCard(n) : _statsCard(n))
//               .toList(),
//         ).animate().fadeIn();
//       },
//     );
...
//             context,
//             MaterialPageRoute(
//               builder: (context) => PushNotification(initialData: n),
//             ),
//           ),
//         ),
...
//               padding: const EdgeInsets.all(20),
//               itemCount:
//                   (selectedNotification!['fcmTokens'] as List?)?.length ?? 0,
//               itemBuilder: (context, index) {
//                 return ListTile(
//                   leading: const Icon(Iconsax.user_tick, color: Colors.teal),
//                   title: Text(
//                     "Token ID: ...${selectedNotification!['fcmTokens'][index].toString().substring(0, 5)}",
//                   ),
...
//           const SizedBox(height: 20),
//           Text(
//             selectedNotification!['title'] ?? "History",
//             style: GoogleFonts.plusJakartaSans(
//               color: Colors.white,
//               fontSize: 20,
//               fontWeight: FontWeight.w800,
...
//           ),
//           Text(
//             "Recipient tracking for ${selectedNotification!['type']}",
//             style: GoogleFonts.inter(color: Colors.white38, fontSize: 12),
//           ),
//         ],
//       ),
...
//     ),
//   );
//   Widget _cardFooter(String label, String time) => Row(
//     mainAxisAlignment: MainAxisAlignment.spaceBetween,
//     children: [
//       Text(
//         label,
...
//             .scale(end: const Offset(1.6, 1.6))
//             .fadeOut(),
//         const SizedBox(width: 16),
//         Text(
//           isSocketConnected ? "CORE ACTIVE" : "CORE OFFLINE",
//           style: GoogleFonts.plusJakartaSans(
//             color: Colors.white,
...
//     ),
//     child: Row(
//       children: [_toggleButton("Scheduled", 0), _toggleButton("History", 1)],
//     ),
//   );
//   Widget _toggleButton(String label, int index) {
//     bool isSelected = activeTab == index;
...
//               style: GoogleFonts.plusJakartaSans(
//                 fontWeight: FontWeight.bold,
//                 color: isSelected ? primaryTeal : Colors.blueGrey,
//               ),
//             ),
//           ),
//         ),
...
class _MasterIntelligenceHubState extends State<MasterIntelligenceHub> {
    ...
    return LayoutBuilder(
        builder: (context, constraints) {
          ...
          return Scaffold(
            key: _scaffoldKey,
            backgroundColor: const Color(0xFFF8FAFC),
            // We keep the drawer for mobile, but remove the leading icon in AppBar
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    // Persistent Side Panel for Desktop
      ...
      builder: (context, snapshot) {
        ...
        return GridView.builder(
          ...
          gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
            ...
            mainAxisExtent: activeTab == 0 ? 220 : 180,
            crossAxisSpacing: 20,
            mainAxisSpacing: 20,
          ),
          itemBuilder: (context, index) {
            final n = currentList[index];
            return activeTab == 0 ? _actionCard(n) : _statsCard(n);
          },
        ).animate().fadeIn();
      },
    );
  ...
  // --- DRAWER (MODIFIED FOR RESPONSIVENESS) ---
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                return ListTile(
                  ...
                  leading: const Icon(Iconsax.user_tick, color: Colors.teal, size: 20),
                  title: Text(
                    "Token ID: ...${selectedNotification!['fcmTokens'][index].toString().substring(0, 5)}",
                    style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600),
                  ),
                  subtitle: const Text("Dispatched via FCM", style: TextStyle(fontSize: 11)),
                ).animate().slideX();
              },
            ),
          ),
  ...
  Widget _drawerHeader(bool isMobile) {
    return Container(
      padding: EdgeInsets.fromLTRB(24, isMobile ? 80 : 40, 24, 32),
      width: double.infinity,
      color: const Color(0xFF0F172A),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          if (!isMobile)
            IconButton(
              ...
              icon: const Icon(Iconsax.close_circle, color: Colors.white24),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                CircleAvatar(radius: 15, backgroundColor: Colors.amber.withOpacity(0.1), child: const Icon(Iconsax.timer_1, color: Colors.amber, size: 16)),
                const SizedBox(width: 12),
                Expanded(child: Text(n['title'] ?? "", maxLines: 1, overflow: TextOverflow.ellipsis, style: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.bold, fontSize: 15))),
              ],
            ),
            const SizedBox(height: 12),
            Expanded(
              child: Text(n['body'] ?? "", maxLines: 3, overflow: TextOverflow.ellipsis, style: GoogleFonts.inter(fontSize: 12, color: Colors.blueGrey)),
            ),
            const Divider(height: 24),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                _cardFooter("Action", _formatToIST(n['scheduledAt'])),
                _editDeleteControls(n),
              ],
            ),
          ],
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Icon(Iconsax.notification_bing, color: primaryTeal, size: 20),
                  const SizedBox(width: 10),
                  Expanded(child: Text(n['title'] ?? "", maxLines: 1, overflow: TextOverflow.ellipsis, style: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.bold, fontSize: 14))),
                ],
              ),
              const Spacer(),
              _buildStatBadges(n['_id']),
              const Spacer(),
              Text("Tap to track recipients", style: TextStyle(fontSize: 10, color: Colors.grey.shade400)),
            ],
          ),
        ),
      ),
  ...
  Widget _buildModernAppBar(bool isLargeScreen) => SliverAppBar(
    ...
    title: Row(
      children: [
        const Icon(Iconsax.chart_21, color: Colors.black, size: 24),
        const SizedBox(width: 12),
        Text("Intelligence Hub", style: GoogleFonts.plusJakartaSans(color: Colors.black, fontWeight: FontWeight.w800, fontSize: 20)),
      ],
    ),
  );
  ...
  Widget _buildSocketMonitor() => Container(
    ...
    child: Row(
      children: [
        Container(width: 8, height: 8, decoration: BoxDecoration(color: isSocketConnected ? Colors.tealAccent : Colors.redAccent, shape: BoxShape.circle))
            .animate(onPlay: (c) => c.repeat()).scale(end: const Offset(1.6, 1.6)).fadeOut(),
        const SizedBox(width: 12),
        Text(isSocketConnected ? "CORE ACTIVE" : "CORE OFFLINE", style: GoogleFonts.plusJakartaSans(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 12, letterSpacing: 1)),
        const Spacer(),
        const Icon(Iconsax.radar, color: Colors.white24, size: 18),
      ],
    ),
  );

  // --- REMAINDER OF HELPERS (Badge, Toggle, etc) ---
  Widget _badge(String val, Color color) => Container(
    ...
    child: Row(
      ...
      children: [
        Icon(color == Colors.green ? Icons.check_circle : Icons.cancel, color: color, size: 12),
        const SizedBox(width: 5),
        Text(val, style: GoogleFonts.inter(fontWeight: FontWeight.bold, fontSize: 11)),
      ],
    ),
  );
  ...
  Widget _editDeleteControls(Map<String, dynamic> n) {
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: [
        IconButton(icon: const Icon(Iconsax.edit, color: Colors.blue, size: 16), onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => PushNotification(initialData: n)))),
        IconButton(icon: const Icon(Iconsax.close_circle, color: Colors.orange, size: 16), onPressed: () => _handleCancel(n['_id'])),
        IconButton(icon: const Icon(Iconsax.trash, color: Colors.red, size: 16), onPressed: () => _confirmDelete(n['_id'])),
      ],
    );
  }
  ...
  Widget _cardFooter(String label, String time) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text(label, style: GoogleFonts.plusJakartaSans(color: const Color(0xFF64748B), fontWeight: FontWeight.bold, fontSize: 11)),
      Text(time, style: GoogleFonts.inter(fontSize: 10, fontWeight: FontWeight.bold, color: primaryTeal)),
    ],
  );
  ...
  Widget _buildSegmentedToggle() => Container(
    ...
    child: Row(children: [_toggleButton("Scheduled", 0), _toggleButton("History", 1)]),
  );
  ...
  Widget _toggleButton(String label, int index) {
    ...
    return Expanded(
      child: GestureDetector(
        onTap: () => setState(() {
          activeTab = index;
          selectedNotification = null; // Reset selection when switching tabs
        }),
        child: AnimatedContainer(
          duration: 300.ms,
          ...
          child: Center(
            child: Text(label, style: GoogleFonts.plusJakartaSans(fontSize: 13, fontWeight: FontWeight.bold, color: isSelected ? primaryTeal : Colors.blueGrey)),
          ),
        ),
      ),
    );
  ...
  Widget _buildWatermarkLogo() => Opacity(
    ...
    child: Column(
      children: [
        const Icon(Iconsax.setting_5, size: 50),
        Text("ANCIL MEDIA", style: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.w900, fontSize: 16, letterSpacing: 4)),
      ],
    ),
  );

  Widget _buildBackgroundAesthetic() => Positioned(top: -100, left: -100, child: CircleAvatar(radius: 200, backgroundColor: primaryTeal.withOpacity(0.04)));
  Widget _emptyState() => Padding(padding: const EdgeInsets.symmetric(vertical: 40), child: Center(child: Text("No Data Available", style: GoogleFonts.inter(color: Colors.grey))));
