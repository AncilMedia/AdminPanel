...
class _HomeContentState extends State<HomeContent> {
  ...
  Future<void> updateLayoutBackend(String layout) async {
    try {
      final url = Uri.parse("$baseUrl/api/homelayout/$organizationId");
      final response = await http.put(url, headers: {"Content-Type": "application/json"}, body: jsonEncode({"layout": layout}));
      if (response.statusCode == 200) {
        showCustomSnackBar(context, "Layout updated to ${layout.toUpperCase()} successfully!", true);
      } else {
        showCustomSnackBar(context, "Failed to update layout.", false);
      }
    } catch (e) {
      showCustomSnackBar(context, "Connection error.", false);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
        builder: (context, boxConstraints) {
          ...
          bool isDesktop = boxConstraints.maxWidth > 1000;
          ...
          return Container(
            ...
            child: SingleChildScrollView(
              ...
              child: isDesktop
                ...
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Expanded(flex: 2, child: _leftContentPanel(groupedItems)),
                  const SizedBox(width: 32),
                  Expanded(flex: 1, child: _rightPreviewPanel()),
                ],
              )
                  : Column( // MOBILE/TABLET VIEW
                children: [
                  _leftContentPanel(groupedItems),
  ...
  Widget _leftContentPanel(Map<String, List<Map<String, dynamic>>> groupedItems) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildHeaderSection(),
  ...
  Widget _buildHeaderSection() {
    return LayoutBuilder(
        builder: (context, constraints) {
          bool smallHeader = constraints.maxWidth < 500;
          return Flex(
            direction: smallHeader ? Axis.vertical : Axis.horizontal,
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            crossAxisAlignment: smallHeader ? CrossAxisAlignment.start : CrossAxisAlignment.center,
            children: [
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text("Home Content", style: GoogleFonts.poppins(fontSize: 24, fontWeight: FontWeight.bold)),
                Text("Drag to reorder items", style: GoogleFonts.poppins(fontSize: 13, color: Colors.blueGrey)),
              ]),
              if (smallHeader) const SizedBox(height: 16),
              ElevatedButton.icon(
                style: ElevatedButton.styleFrom(
                  backgroundColor: Colors.blueAccent,
                  foregroundColor: Colors.white,
  ...
  Widget _buildFilterBar() {
    return Container(
      ...
      child: LayoutBuilder(
          builder: (context, constraints) {
            bool wrapFilter = constraints.maxWidth < 450;
            return Wrap(
              ...
              children: [
                ...
                if (!wrapFilter) const SizedBox(width: 16),
                SizedBox(
                  width: wrapFilter ? double.infinity : 200,
                  child: TextField(
                    onChanged: (v) => setState(() => searchQuery = v),
                    decoration: const InputDecoration(hintText: "Search records...", border: InputBorder.none, contentPadding: EdgeInsets.symmetric(horizontal: 10)),
                  ),
                ),
              ],
            );
        ...
        children: [
          ...
          Container(
            ...
            child: ClipRRect(
              ...
              child: Column(
                children: [
                  Expanded(child: Container(color: Colors.white, child: _renderMobileLayout())),
                  _mobileBottomNavPreview(),
                ],
              ),
            ),
    ...
    return ListView.builder(
      ...
      itemBuilder: (_, i) => Container(
        ...
        child: Row(children: [
          ...
          _buildItemImage(items[i]['image']),
          const SizedBox(width: 8),
          Expanded(child: Text(items[i]['title'] ?? '', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis)),
        ]),
      ),
    );
  }
  ...
  Widget _mobileRowPreview(List<Map<String, dynamic>> items) {
    return SingleChildScrollView(
      ...
      child: Row(children: items.map((item) => Container(
        ...
        margin: const EdgeInsets.only(right: 8),
        child: Column(children: [
          ClipRRect(borderRadius: BorderRadius.circular(8), child: SizedBox(height: 60, width: 80, child: _previewImage(item))),
          const SizedBox(height: 4),
          Text(item['title'] ?? '', maxLines: 1, style: const TextStyle(fontSize: 8), overflow: TextOverflow.ellipsis),
        ]),
      )).toList()),
    );
  }
  ...
  Widget _previewImage(Map<String, dynamic> item) {
    ...
    if (img != null && img.toString().isNotEmpty) {
      return Image.network(img.toString(), fit: BoxFit.cover, errorBuilder: (_, __, ___) => Container(color: Colors.grey));
    }
    return Container(color: Colors.grey);
  }
  ...
  Widget _mobileBottomNavPreview() {
    if (navLoading) return const SizedBox(height: 50, child: Center(child: CircularProgressIndicator(strokeWidth: 2)));
    return Container(
      height: 50,
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceAround,
        children: navItems.map((item) {
          return _navItem(getIcon(item['icon'] ?? ''), item['label'] ?? '', navItems.indexOf(item) == 0);
        }).toList(),
      ),
    );
  }
  ...
  Widget _navItem(IconData icon, String label, bool selected) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 2),
        Text(label, style: TextStyle(fontSize: 7, color: selected ? Colors.blueAccent : Colors.grey)),
      ],
    );
  }
  ...
  IconData getIcon(String name) {
    switch (name.toLowerCase()) {
      ...
      case "clock": return Iconsax.clock;
      case "video": return Iconsax.video;
      case "book": return Iconsax.book;
      case "wallet_check": return Iconsax.wallet_check;
