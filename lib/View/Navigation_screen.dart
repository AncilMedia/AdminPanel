...
class _NavigationFormPageState extends State<NavigationFormPage> {
  ...
  final Color accentCyan = const Color(0xFF00BCD4); // Matches image_57bf35.png
  ...
  void initState() {
    ...
    labelCtrl = TextEditingController(text: widget.navData?['label'] ?? '');
    iconCtrl = TextEditingController(text: widget.navData?['icon'] ?? '');
    orderCtrl = TextEditingController(text: widget.navData?['order']?.toString() ?? '0');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          widget.navData == null ? "Create Navigation" : "Update Item",
          style: GoogleFonts.plusJakartaSans(color: slate900, fontWeight: FontWeight.w800, fontSize: 18),
        ),
        iconTheme: IconThemeData(color: slate900),
      ),
      body: LayoutBuilder(
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  _buildSectionTitle("Configuration Details"),
                  const SizedBox(height: 12),
                  _buildFormGrid(isDesktop || isTablet),
                  const SizedBox(height: 32),
                  _buildSectionTitle("Organization Assignment"),
                  const SizedBox(height: 12),
                  _buildOrgDropdown(),
                  if (selectedOrgId != null) ...[
                    const SizedBox(height: 32),
                    _buildNavList("Currently Assigned Items", assignedNavs, true, isDesktop),
                    if (isAdmin && unassignedNavs.isNotEmpty) ...[
                      const SizedBox(height: 32),
                      _buildNavList("Available to Assign", unassignedNavs, false, isDesktop),
                    ],
                  ],
  ...
  Widget _buildSectionTitle(String title) {
    return Text(
      title.toUpperCase(),
      style: GoogleFonts.plusJakartaSans(fontSize: 11, fontWeight: FontWeight.w800, color: slate500, letterSpacing: 1),
    );
  }

  Widget _buildFormGrid(bool isWide) {
    return Container(
      padding: const EdgeInsets.all(24),
      decoration: BoxDecoration(
        color: Colors.white,
        ...
        children: [
          if (isWide)
            Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Expanded(child: _buildTextField("Label Name", labelCtrl, Iconsax.text_block)),
            ...
            _buildTextField("Label Name", labelCtrl, Iconsax.text_block),
            const SizedBox(height: 16),
            _buildTextField("Icon Key", iconCtrl, Iconsax.category),
            const SizedBox(height: 16),
            _buildTextField("Sort Order", orderCtrl, Iconsax.sort, isNum: true),
          ],
          const SizedBox(height: 24),
          if (isWide)
            Row(
  ...
  Widget _buildActiveToggle() {
    return Row(
      ...
      children: [
        Text("Active Status", style: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.w700, color: slate900)),
  ...
  Widget _buildTextField(String label, TextEditingController ctrl, IconData icon, {bool isNum = false}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(label, style: GoogleFonts.plusJakartaSans(fontSize: 12, fontWeight: FontWeight.w700, color: slate500)),
        const SizedBox(height: 6),
        TextFormField(
          controller: ctrl,
          keyboardType: isNum ? TextInputType.number : TextInputType.text,
  ...
  Widget _buildTypeSelector() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text("Navigation Type", style: GoogleFonts.plusJakartaSans(fontSize: 12, fontWeight: FontWeight.w700, color: slate500)),
        const SizedBox(height: 6),
        DropdownButtonFormField<String>(
          value: type,
          decoration: InputDecoration(
  ...
  Widget _buildOrgDropdown() {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 16),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: slate200)),
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          value: organizations.any((o) => o['_id'].toString() == selectedOrgId) ? selectedOrgId : null,
          isExpanded: true,
          hint: Text("Select Organization", style: GoogleFonts.plusJakartaSans()),
          items: organizations.map((org) => DropdownMenuItem(
            value: org['_id'].toString(),
            child: Text(org['name'].toString(), style: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.w600)),
          )).toList(),
          onChanged: (val) {
            setState(() => selectedOrgId = val);
            fetchNavsForOrg();
  ...
  Widget _buildNavList(String title, List<Map<String, dynamic>> items, bool isAssigned, bool isDesktop) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildSectionTitle(title),
        ...
        Wrap(
          spacing: 12,
          runSpacing: 12,
          children: items.map((nav) => Container(
            ...
            child: ListTile(
              title: Text(nav['label'] ?? '', style: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.w700, fontSize: 14)),
              subtitle: Text(nav['type'] ?? '', style: GoogleFonts.plusJakartaSans(fontSize: 12, color: slate500)),
              trailing: isAdmin
                  ? Switch.adaptive(activeColor: primaryIndigo, value: isAssigned, onChanged: (_) => toggleNav(nav['_id'].toString()))
                  : Icon(isAssigned ? Icons.check_circle : Icons.circle_outlined, color: isAssigned ? Colors.green : slate200),
            ),
          )).toList(),
        ),
      ],
  ...
  Widget _buildBottomAction() {
    return Container(
      ...
      child: ElevatedButton(
        onPressed: loading ? null : () {}, // Add submit logic here
        style: ElevatedButton.styleFrom(
