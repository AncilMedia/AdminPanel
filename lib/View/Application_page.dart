...
class _ApplicationPageState extends State<ApplicationPage> {
  ...
  final Color primaryBrand = const Color(0xFF0F172A);
  final Color accentAction = const Color(0xFF0D9488);
  ...
  Widget _buildHeader(bool isDesktop) {
    return SliverToBoxAdapter(
      child: Padding(
        padding: EdgeInsets.fromLTRB(24, isDesktop ? 60 : 40, 24, 20),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(
  ...
  Widget _buildAppList(bool isDesktop) {
    if (_apps.isEmpty) {
      return SliverFillRemaining(
        child: Center(child: Text("No applications found", style: GoogleFonts.plusJakartaSans(color: slate500))),
      );
    ...
    return SliverPadding(
      ...
      sliver: SliverList(
        delegate: SliverChildBuilderDelegate(
              (context, index) {
            final app = _apps[index];
            return isDesktop ? _buildDesktopRow(app) : _buildMobileCard(app);
          },
          childCount: _apps.length,
        ),
      ),
  ...
  Widget _buildDesktopRow(dynamic app) {
    return Container(
      margin: const EdgeInsets.only(bottom: 12),
      padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 18),
      decoration: BoxDecoration(
        ...
        children: [
          ...
          Expanded(
            ...
            child: Align(
              ...
              child: _deleteIconButton(app['_id']),
            ),
          ),
        ],
      ),
  ...
  Widget _buildMobileCard(dynamic app) {
    return Container(
              ...
              Row(
                children: [
                  CircleAvatar(
                    radius: 14,
                    backgroundColor: accentAction.withOpacity(0.1),
                    child: Icon(Iconsax.mobile, size: 14, color: accentAction),
                  ),
                  const SizedBox(width: 10),
                  Text(
                    app['appName'] ?? 'Unnamed',
                    style: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.w700, color: primaryBrand, fontSize: 16),
                  ),
                ],
              ),
              _deleteIconButton(app['_id']),
            ],
          ),
          const SizedBox(height: 16),
          _mobileInfoRow("Organizations", _getOrgNames(app)),
          const SizedBox(height: 8),
          _mobileInfoRow("Created", app['createdAt']?.substring(0, 10) ?? ''),
        ],
      ),
    );
  }
  ...
  Widget _mobileInfoRow(String label, String value) {
    return Column(
      ...
      children: [
        Text(
          label.toUpperCase(),
          style: GoogleFonts.plusJakartaSans(fontSize: 10, fontWeight: FontWeight.w800, color: slate500, letterSpacing: 0.5),
        ),
        const SizedBox(height: 2),
        Text(
          value,
          style: GoogleFonts.plusJakartaSans(color: primaryBrand, fontSize: 14, fontWeight: FontWeight.w500),
        ),
      ],
    );
  }

  Widget _deleteIconButton(String id) {
    return IconButton(
      onPressed: () => _deleteApp(id),
      icon: const Icon(Iconsax.trash, color: Colors.redAccent, size: 20),
      style: IconButton.styleFrom(
        backgroundColor: Colors.redAccent.withOpacity(0.1),
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
