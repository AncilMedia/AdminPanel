...
class _AnalyticsPageState extends State<AnalyticsPage> with TickerProviderStateMixin {
    ...
    return Scaffold(
      ...
      body: isLoading
        ...
        builder: (context, constraints) {
          return _buildDashboardBody(constraints);
        },
      ),
    );
  }

  Widget _buildDashboardBody(BoxConstraints constraints) {
    final chartData = List<Map<String, dynamic>>.from(analyticsData?['chartData'] ?? []);
    final mediaItems = List<Map<String, dynamic>>.from(analyticsData?['mediaList'] ?? []);
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _staggeredEntry(0.0, 0.4, child: _buildFilterBar(isMobile)),
          const SizedBox(height: 32),
          _staggeredEntry(0.1, 0.5, child: _buildQuickStats(mediaItems, isMobile)),
          const SizedBox(height: 32),
          _staggeredEntry(0.2, 0.6, child: _buildChartSection(chartData)),
          const SizedBox(height: 32),
          _staggeredEntry(0.3, 0.7, child: _buildMediaTable(mediaItems, isMobile)),
        ],
      ),
    );
  }
  ...
  Widget _buildQuickStats(List mediaItems, bool isMobile) {
    int totalPlays = mediaItems.fold(0, (sum, item) => sum + (item['plays'] as int));
    int totalViewers = mediaItems.fold(0, (sum, item) => sum + (item['uniqueViewers'] as int));
    ...
    if (isMobile) {
      return Column(
        children: [
          _statCard("Total Plays", _fmt(totalPlays), Iconsax.play_circle, Colors.blue, false),
          const SizedBox(height: 16),
          _statCard("Reach", _fmt(totalViewers), Iconsax.personalcard, Colors.purple, false),
          const SizedBox(height: 16),
          _statCard("Media Items", mediaItems.length.toString(), Iconsax.video_octagon, Colors.orange, false),
        ],
      );
    }
    ...
    return Row(
      children: [
        _statCard("Total Plays", _fmt(totalPlays), Iconsax.play_circle, Colors.blue, true),
        const SizedBox(width: 20),
        _statCard("Reach", _fmt(totalViewers), Iconsax.personalcard, Colors.purple, true),
        const SizedBox(width: 20),
        _statCard("Media Items", mediaItems.length.toString(), Iconsax.video_octagon, Colors.orange, true),
      ],
    );
  }
        ...
        children: [
          ...
          SizedBox(
            ...
            child: SfCartesianChart(
              ...
              primaryXAxis: CategoryAxis(
                majorGridLines: const MajorGridLines(width: 0),
                labelStyle: baseStyle.copyWith(fontSize: 10),
                labelRotation: 45, // Rotated for small screens
              ),
              primaryYAxis: NumericAxis(
                axisLine: const AxisLine(width: 0),
                majorTickLines: const MajorTickLines(size: 0),
              ...
              legend: Legend(
                  ...
                  textStyle: baseStyle.copyWith(fontSize: 11)),
              tooltipBehavior: _tooltipBehavior,
              series: <CartesianSeries>[
                _buildStackedSeries(chartData, 'ios', 'iOS', Colors.amber),
                _buildStackedSeries(chartData, 'android', 'Android', Colors.blue),
  ...
  Widget _buildMediaTable(List mediaItems, bool isMobile) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text("Content Performance", style: baseStyle.copyWith(fontWeight: FontWeight.bold, fontSize: 16)),
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                ...
                title: Text(item['title'] ?? "Untitled Content",
                    style: baseStyle.copyWith(fontWeight: FontWeight.bold, fontSize: isMobile ? 12 : 14),
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis),
                subtitle: Text("Views: ${item['plays']}",
                    style: baseStyle.copyWith(fontSize: 11, color: Colors.blueGrey.shade400)),
                trailing: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  crossAxisAlignment: CrossAxisAlignment.end,
                  children: [
                    Text(item['totalPlayTime'].toString(),
                        style: baseStyle.copyWith(fontWeight: FontWeight.bold, color: Colors.indigo, fontSize: isMobile ? 12 : 14)),
                    if (!isMobile) Text("Total Time", style: baseStyle.copyWith(fontSize: 10, color: Colors.grey.shade400)),
                  ],
                ),
              );
            },
  ...
  Widget _statCard(String label, String value, IconData icon, Color color, bool isExpanded) {
            ...
            child: Column(
                ...
                children: [
                  ...
                  FittedBox(
                    ...
                    child: Text(value, style: baseStyle.copyWith(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.blueGrey.shade900)),
                  ),
                ]),
          ),
        ],
  ...
  StackedColumnSeries<Map<String, dynamic>, String> _buildStackedSeries(List<Map<String, dynamic>> data, String key, String name, Color color) {
    return StackedColumnSeries<Map<String, dynamic>, String>(
      ...
      name: name,
      color: color,
      width: 0.5, // Increased width for better visibility on small screens
      borderRadius: const BorderRadius.vertical(top: Radius.circular(6)),
    );
  }
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Icon(Iconsax.calendar, size: 16, color: Colors.indigo),
          const SizedBox(width: 10),
          Text(date != null ? DateFormat("dd MMM yy").format(date) : label, style: baseStyle.copyWith(fontSize: 13, color: Colors.blueGrey)),
        ]),
      ),
    );
  }
