...
class _LibraryPageState extends State<LibraryPage> {
        ...
        return SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 32),
              _buildFeatureSection(isMobile, isTablet),
              const SizedBox(height: 40),
              _buildSectionTitle("Recent Media Items", Iconsax.video_play),
              _buildMediaItemList(isMobile),
              const SizedBox(height: 48),
              _buildSectionTitle("Media Series", Iconsax.folder_2),
              _buildSeriesGrid(constraints.maxWidth),
              const SizedBox(height: 100),
            ],
          ),
        );
  ...
  Widget _buildCreateMenu() {
    return PopupMenuButton<String>(
      onSelected: (value) async {
        if (value == "item") {
          await showCreateMediaItemDialog(context, _itemService, _seriesService);
        } else if (value == "series") {
          await showCreateMediaSeriesDialog(context, _seriesService);
        }
        _fetchUserMedia();
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(12),
          boxShadow: [
            BoxShadow(color: Colors.indigo.withOpacity(0.3), blurRadius: 12, offset: const Offset(0, 4)),
          ],
        ),
        child: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Icon(Iconsax.add_square, color: Colors.white, size: 20),
            const SizedBox(width: 10),
            Text("Create", style: GoogleFonts.poppins(color: Colors.white, fontWeight: FontWeight.w600)),
            const Icon(Icons.keyboard_arrow_down, color: Colors.white),
          ],
        ),
      ),
  ...
  Widget _buildFeatureSection(bool isMobile, bool isTablet) {
    if (isMobile) {
      return Column(
        children: [
          _featureCard("Bulk Metadata Editor", "Tag topics, scriptures, and speakers instantly.", Iconsax.magicpen, Colors.blue.shade900, "Get Started", true),
          const SizedBox(height: 16),
          _featureCard("Quick Upload", "Drop files to auto-create items.", Iconsax.cloud_add, Colors.teal.shade700, "Upload", true),
        ],
      );
    }
    return Row(
      children: [
        Expanded(
          flex: 2,
          child: _featureCard("Bulk Metadata Editor", "Tag topics, scriptures, and speakers across multiple items instantly.", Iconsax.magicpen, Colors.blue.shade900, "Get Started", false),
        ),
        const SizedBox(width: 24),
        Expanded(
          flex: 1,
          child: _featureCard("Quick Upload", "Drop files to auto-create items.", Iconsax.cloud_add, Colors.teal.shade700, "Upload", false),
        ),
      ],
    );
  }

  Widget _featureCard(String title, String desc, IconData icon, Color color, String btnText, bool isMobile) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(icon, color: Colors.white.withOpacity(0.8), size: isMobile ? 30 : 40),
              const SizedBox(width: 16),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(title, style: GoogleFonts.poppins(color: Colors.white, fontWeight: FontWeight.bold, fontSize: isMobile ? 14 : 16)),
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Icon(icon, size: 20, color: Colors.indigo),
          const SizedBox(width: 12),
          Text(title, style: GoogleFonts.poppins(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.blueGrey.shade800)),
        ],
      ),
    );
  }

  Widget _buildMediaItemList(bool isMobile) {
    return Container(
      decoration: BoxDecoration(
        color: Colors.white,
        borderRadius: BorderRadius.circular(24),
      ...
      child: _mediaItems.isEmpty
          ? const Padding(padding: EdgeInsets.all(20), child: Center(child: Text("No media items found")))
          : ListView.separated(
        shrinkWrap: true,
        physics: const NeverScrollableScrollPhysics(),
        itemCount: _mediaItems.length,
            ...
            child: Row(
              children: [
                Expanded(
                  child: InkWell(
                    borderRadius: BorderRadius.circular(24),
                    onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => LibraryDetails(mediaitemid: item["_id"]))),
                    child: Row(
                      children: [
                        Container(
                          ...
                          child: ClipRRect(
                            ...
                            child: item["thumbnailUrl"] != null
                                ...
                                : const Icon(Iconsax.video, color: Colors.indigo, size: 30),
                          ),
                        ),
                        const SizedBox(width: 16),
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text(item["title"] ?? "Untitled",
                                  maxLines: 1,
                                  overflow: TextOverflow.ellipsis,
                                  style: GoogleFonts.poppins(fontWeight: FontWeight.bold, fontSize: isMobile ? 13 : 15, color: Colors.blueGrey.shade900)),
                              const SizedBox(height: 4),
                              Row(
                                children: [
                                  const Icon(Iconsax.folder_open, size: 14, color: Colors.indigo),
                                  const SizedBox(width: 4),
                                  Flexible(
                                    child: Text(item["seriesId"]?["title"] ?? "No Series",
                                        maxLines: 1, overflow: TextOverflow.ellipsis, style: GoogleFonts.poppins(fontSize: 12, color: Colors.blueGrey.shade600)),
                                  ),
                                ],
                              ),
                              if (!isMobile) ...[
                                ...
                                Row(
                                  children: [
                                    const Icon(Iconsax.calendar_1, size: 14, color: Colors.blueGrey),
                                    const SizedBox(width: 4),
                                    Text("${item["createdAt"]?.toString().split('T').first ?? 'N/A'}",
                ...
                PopupMenuButton<String>(
                  ...
                  onSelected: (value) {
                    if (value == "delete") _deleteMediaItem(item);
                  },
                  itemBuilder: (_) => [
                    const PopupMenuItem(
                        value: "delete",
  ...
  Widget _buildSeriesGrid(double width) {
            ...
            children: [
              Expanded(
                child: ClipRRect(
                  ...
                  child: series["thumbnail"] != null
                      ? Image.network(series["thumbnail"], width: double.infinity, fit: BoxFit.cover)
                      : Container(color: Colors.indigo.withOpacity(0.05), child: const Center(child: Icon(Iconsax.folder_open, color: Colors.indigo, size: 20))),
                ),
              ),
              Padding(
                  ...
                  children: [
                    Expanded(
                      child: Text(series["title"] ?? "Untitled",
                          maxLines: 1, overflow: TextOverflow.ellipsis, style: GoogleFonts.poppins(fontWeight: FontWeight.w600, fontSize: 11, color: Colors.blueGrey.shade800)),
                    ),
                    SizedBox(
                      width: 20,
                      child: PopupMenuButton<String>(
                        padding: EdgeInsets.zero,
                        icon: const Icon(Iconsax.more, size: 14, color: Colors.grey),
                        onSelected: (v) => _deleteSeries(series),
                        itemBuilder: (_) => [const PopupMenuItem(value: "del", child: Text("Delete", style: TextStyle(fontSize: 12, color: Colors.red)))],
                      ),
                    ),
                  ],
                ),
