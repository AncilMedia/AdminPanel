...
class _HomePageState extends State<HomePage> {
  ...
  List<Map<String, dynamic>> get monthlySeries {
    ...
    for (var m in raw) {
      final monthStr = (m['monthLabel'] ?? m['month'] ?? '').toString();
      DateTime dt;
      try { dt = df.parse(monthStr); } catch (_) { continue; }
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildResponsiveStatCards(isMobile),
                ...
                if (isMobile) ...[
                  ...
                  const SizedBox(height: 24),
                  _buildCircularChartSection(),
                ] else ...[
                  Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Expanded(flex: 3, child: _buildMainChartSection()),
                      const SizedBox(width: 24),
                      Expanded(flex: 2, child: _buildCircularChartSection()),
                    ],
                  ),
                ],
              ],
  ...
  Widget _buildResponsiveStatCards(bool isMobile) {
    // This Row ensures all three cards stay in one line regardless of screen size
    return Row(
      children: [
        Expanded(
        ...
        const SizedBox(width: 12), // Reduced spacing for mobile compatibility
        Expanded(
          child: _buildStatCard(
            'Peak Plays',
            _fmt(peakPlays),
  ...
  Widget _buildStatCard(String title, String value, IconData icon, List<Color> gradient, {String? subtitle, required bool isMobile}) {
    return Container(
      padding: EdgeInsets.all(isMobile ? 12 : 20), // Adaptive padding
      ...
      child: Row(
        children: [
          // Hide or shrink icon on very small screens to save space
          Container(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                // FittedBox ensures the number scales down instead of clipping
                FittedBox(
                ...
                if (subtitle != null && !isMobile) // Hide subtitle on mobile to keep row clean
                  Text(
                    subtitle,
                    style: GoogleFonts.poppins(
                      fontSize: 11,
  ...
  Widget _buildCircularChartSection() {
    return _buildDashboardCard(
      title: 'Platforms',
      subtitle: 'Device distribution',
      child: SizedBox(height: 300, child: _buildCircularChart()),
  ...
  // Widget _buildStatCard(String title, String value, IconData icon, List<Color> gradient, {String? subtitle, double? width}) {
  //   return Container(
  //     width: width, // Handles mobile expansion
  //     constraints: BoxConstraints(minWidth: 280), // Prevents cards from getting too squished on desktop
  //     padding: const EdgeInsets.all(24),
  //     decoration: BoxDecoration(
  //         color: Colors.white,
  ...
  //         ),
  //         const SizedBox(width: 20),
  //         Expanded( // Added expanded to prevent overflow on very small screens
  //           child: Column(
  //             crossAxisAlignment: CrossAxisAlignment.start,
  //             children: [
  //               Text(title, style: GoogleFonts.poppins(fontSize: 14, color: Colors.blueGrey), overflow: TextOverflow.ellipsis),
  //               Text(value, style: GoogleFonts.poppins(fontSize: 22, fontWeight: FontWeight.bold)),
  //               if (subtitle != null)
  //                 Text(subtitle, style: GoogleFonts.poppins(fontSize: 12, color: Colors.green, fontWeight: FontWeight.w600), overflow: TextOverflow.ellipsis),
  //             ],
  //           ),
  //         ),
  //       ],
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(
          ...
          children: [
            Expanded( // Prevents title overlap with button on narrow screens
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(title, style: GoogleFonts.poppins(fontSize: 18, fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis),
                Text(subtitle, style: GoogleFonts.poppins(fontSize: 13, color: Colors.blueGrey), overflow: TextOverflow.ellipsis),
              ]),
            ),
            if (action != null) action,
          ],
  ...
  Widget _buildMainChart() {
    return SfCartesianChart(
      ...
      primaryXAxis: CategoryAxis(
        ...
        labelRotation: 45, // Rotates labels to avoid overlap on small screens
      ),
      primaryYAxis: NumericAxis(axisLine: const AxisLine(width: 0), majorTickLines: const MajorTickLines(size: 0)),
      tooltipBehavior: _tooltipBehavior,
      series: <CartesianSeries<Map<String, dynamic>, String>>[
  ...
  Widget _buildCircularChart() {
    return SfCircularChart(
      legend: Legend(
        ...
        overflowMode: LegendItemOverflowMode.wrap, // Wraps legend items for mobile
      ),
      series: <CircularSeries>[
        DoughnutSeries<MapEntry<String, int>, String>(
          dataSource: deviceDistribution.entries.toList(),
