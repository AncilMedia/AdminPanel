...
class _ManageRolesState extends State<ManageRoles> {
  ...
  final Color accentAction = const Color(0xFF6366F1);
  ...
  Widget _buildHeader(BuildContext context) {
    // Determine screen size for font responsiveness
    final bool isSmall = MediaQuery.of(context).size.width < 600;

    return Container(
      padding: EdgeInsets.fromLTRB(24, 32, 16, 20),
      decoration: BoxDecoration(
        color: Colors.white,
        borderRadius: const BorderRadius.vertical(top: Radius.circular(32)),
        border: Border(bottom: BorderSide(color: slate200)),
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Icon(Iconsax.security_user, color: accentAction, size: isSmall ? 18 : 22),
          ),
          const SizedBox(width: 16),
          ...
          IconButton(
            onPressed: () => Navigator.pop(context),
            icon: Icon(Icons.close, color: slate400, size: isSmall ? 20 : 24),
          ),
        ],
      ),
    );
          ...
          child: Column(
            ...
            children: [
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Flexible( // Allows badge to shrink on narrow screens
                    child: Container(
                      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                      decoration: BoxDecoration(
                        color: primaryBrand.withOpacity(0.05),
              ...
              const SizedBox(height: 16),
              FittedBox( // Ensures date/icon fits in one row regardless of width
                child: Row(
                  children: [
                    Icon(Iconsax.calendar_1, size: 14, color: slate400),
                    const SizedBox(width: 6),
...
void openManageRolesSheet(BuildContext context) {
  showGeneralDialog(
    ...
    pageBuilder: (context, anim, __) {
      ...
      double panelWidth;
      if (screenWidth > 1200) {
        panelWidth = screenWidth * 0.3; // Desktop
      } else if (screenWidth > 800) {
        panelWidth = screenWidth * 0.45; // Tablet
      } else {
        panelWidth = screenWidth * 0.9; // Mobile
      }
