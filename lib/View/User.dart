...
// //               Column(
// //                 children: [
// //                   _buildModernHeader(constraints.maxWidth),
// //                   Expanded(
// //                     child: FutureBuilder<List<UserModel>>(
// //                       future: userListFuture,
// //                       builder: (context, snapshot) {
...
// //                             mainAxisExtent: 185, // Fixed height for cards
// //                             crossAxisSpacing: 16,
// //                             mainAxisSpacing: 0,
// //                           ),
// //                           itemCount: users.length,
...
// //             children: [
// //               Text("MANAGEMENT", style: GoogleFonts.poppins(fontSize: 12, color: Colors.cyan.shade800, fontWeight: FontWeight.bold, letterSpacing: 2)),
// //               Text("User Accounts", style: GoogleFonts.poppins(fontSize: width < 400 ? 22 : 28, fontWeight: FontWeight.w800, color: Colors.blueGrey.shade900)),
// //               const SizedBox(height: 20),
...
// //                           style: GoogleFonts.poppins(fontWeight: FontWeight.bold, fontSize: 14, color: Colors.blueGrey.shade900),
...
// //       child: Text(label, style: GoogleFonts.poppins(color: color, fontSize: 9, fontWeight: FontWeight.bold, letterSpacing: 0.5)),
// //     );
// //   }
// //
// //   Widget _buildRoleDropdown(BuildContext context) {
...
// //         value: user.role,
// //         underline: const SizedBox(),
// //         icon: const Icon(Iconsax.user_edit, size: 12),
// //         style: GoogleFonts.poppins(fontSize: 10, color: Colors.blueGrey, fontWeight: FontWeight.w600),
// //         items: availableRoles.map((r) => DropdownMenuItem(value: r, child: Text(r.toUpperCase()))).toList(),
// //         onChanged: (newRole) async {
// //           if (newRole != null && newRole != user.role) {
// //             final success = await UserController.updateUserRole(authState: Provider.of<AuthState>(context, listen: false), userId: user.id, newRole: newRole);
...
// //
// //   Widget _circleAction(IconData icon, Color color, VoidCallback onTap) {
// //     return InkWell(onTap: onTap, child: Container(padding: const EdgeInsets.all(6), decoration: BoxDecoration(color: color.withOpacity(0.1), shape: BoxShape.circle), child: Icon(icon, color: color, size: 20)));
// //   }
// //
// //   Widget _blockButton(BuildContext context) {
// //     bool disabled = user.approved == false || isMe;
...
// //           decoration: BoxDecoration(color: user.blocked ? Colors.red.shade50 : Colors.orange.shade50, borderRadius: BorderRadius.circular(8)),
// //           child: Text(user.blocked ? "UNBLOCK" : "BLOCK", style: TextStyle(color: user.blocked ? Colors.red : Colors.orange, fontSize: 9, fontWeight: FontWeight.bold)),
// //         ),
// //       ),
// //     );
// //   }
...
// //   Widget _actionMenu(BuildContext context) {
// //     return PopupMenuButton<String>(
// //       icon: const Icon(Iconsax.more, color: Colors.blueGrey, size: 18),
// //       onSelected: (val) {
// //         if (val == 'edit') showDialog(context: context, builder: (_) => EditUserDialog(user: user, onSave: onRefresh));
// //         else if (val == 'delete') showDialog(context: context, builder: (_) => DeleteUser(user: user, onDelete: onRefresh));
// //       },
// //       itemBuilder: (ctx) => [
// //         const PopupMenuItem(value: 'edit', child: Row(children: [Icon(Iconsax.edit, size: 16), SizedBox(width: 8), Text("Edit", style: TextStyle(fontSize: 13))])),
// //         if (!isMe) const PopupMenuItem(value: 'delete', child: Row(children: [Icon(Iconsax.trash, size: 16, color: Colors.red), SizedBox(width: 8), Text("Delete", style: TextStyle(color: Colors.red, fontSize: 13))])),
// //       ],
// //     );
// //   }
// //
...
//                           gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
//                             crossAxisCount: isWide ? 3 : (isTablet ? 2 : 1),
//                             mainAxisExtent: 220, // Increased height for the new Last Seen row
//                             crossAxisSpacing: 16,
//                             mainAxisSpacing: 0,
//                           ),
//                           itemCount: users.length,
...
//                                   style: GoogleFonts.poppins(fontSize: 9, color: isOnline ? Colors.green : Colors.grey.shade600, fontWeight: FontWeight.w800, letterSpacing: 0.5)
//                               ),
//                             ),
//                           ],
//                         ),
...
//                     child: Row(
//                       children: [
//                         Icon(Iconsax.clock, size: 14, color: Colors.blueGrey.shade300),
//                         const SizedBox(width: 8),
//                         Text(
//                           "Last activity:",
//                           style: GoogleFonts.poppins(fontSize: 10, color: Colors.blueGrey.shade400, fontWeight: FontWeight.w500),
//                         ),
//                         const Spacer(),
//                         Text(
//                           isOnline ? "Active now" : _formatLastSeen(lastSeen),
//                           style: GoogleFonts.poppins(fontSize: 10, color: isOnline ? Colors.cyan.shade700 : Colors.blueGrey.shade700, fontWeight: FontWeight.bold),
//                         ),
//                       ],
//                     ),
//                   ),
...
class _UserPageState extends State<UserPage> {
  ...
  void _fetchUsers() {
    ...
    String? approved = (selectedApprovalStatus == 'Approved')
        ? 'true'
        : (selectedApprovalStatus == 'Rejected'
              ? 'false'
              : (selectedApprovalStatus == 'Pending' ? 'null' : null));
                ...
                Column(
                  children: [
                    ...
                    Row(
                      children: [
                        Expanded(
                          child: _headerDropdown(selectedRole, roles, "Role"),
                        ),
...
class UserModernCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // ================= UPDATED =================
    final bool isOnline =
        presenceStatus == "online" || user.isOnline;
  ...
  Widget _buildStatusBadge(UserModel user) {
    Color color = user.approved == true
        ? Colors.green
        : (user.approved == false
        ? Colors.red
        : Colors.orange);

    String label = user.approved == true
        ? "ACTIVE"
        : (user.approved == false
        ? "REJECTED"
        : "PENDING");
