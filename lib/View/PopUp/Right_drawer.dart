...
//   final ItemModel? rootItem;
//   final bool isInSublist;
//   final DrawerSelection? initialSelection; // New parameter to force a type
//
//   const CustomRightDrawer({
//     super.key,
//     this.onAddItemToHome,
...
//     this.rootItem,
//     required this.isInSublist,
//     this.initialSelection, // Added to constructor
//   });
//
//   @override
//   State<CustomRightDrawer> createState() => _CustomRightDrawerState();
...
//
// class _CustomRightDrawerState extends State<CustomRightDrawer> with SingleTickerProviderStateMixin {
//   late DrawerSelection selected; // Changed to late to initialize in initState
//   List<ListModel> recentLists = [];
//   bool showCreateForm = false;
//   String newTitle = '';
//   String newSubtitle = '';
...
//   void initState() {
//     super.initState();
//     // Initialize selection: logic to lock it to Event if passed from Event page
//     selected = widget.initialSelection ?? DrawerSelection.list;
//     _loadLists();
//     Future.delayed(const Duration(milliseconds: 300), () {
//       if (mounted) setState(() => animateButton = true);
//     });
...
//         crossAxisAlignment: CrossAxisAlignment.start,
//         children: [
//           // HIDDEN TYPE SELECTOR IF INITIAL SELECTION IS PROVIDED
...
//                   if (result != null) setState(() => pickedImage = result.files.first.bytes);
//                 },
//                 child: Container(
//                   width: double.infinity, height: 180,
//                   decoration: BoxDecoration(color: Colors.grey.shade50, borderRadius: BorderRadius.circular(15), border: Border.all(color: Colors.blueAccent.withOpacity(0.2))),
...
class _CustomRightDrawerState extends State<CustomRightDrawer> with SingleTickerProviderStateMixin {
  ...
  String? _generateRRule() {
    ...
    if (selectedMonths.isNotEmpty) rule += ";BYMONTH=${selectedMonths.join(',')}";
    if (recurrenceUntil != null) {
      rule += ";UNTIL=${recurrenceUntil!.toIso8601String().split('T')[0].replaceAll('-', '')}T235959Z";
    }
    return rule;
  ...
  Future<void> _handleCreateNewList() async {
    ...
    try {
      ItemModel newItem;
      String? rRule = (selected == DrawerSelection.event) ? _generateRRule() : null;
      ...
      if (widget.isInSublist) {
        final newList = await ListController.createList(
          ...
          type: selected.name,
          url: (selected == DrawerSelection.link) ? newUrl.trim() : null,
        );
        newItem = ItemModel(id: newList.id, title: newList.title, subtitle: newList.subtitle, image: newList.image, type: newList.type ?? selected.name, parentId: newList.parentId, index: newList.index);
      } else {
        newItem = await ItemService.createItem(
          ...
          type: selected.name,
          url: (selected == DrawerSelection.link) ? newUrl.trim() : null,
          startDateTime: (selected == DrawerSelection.event) ? startDateTime?.toIso8601String() : null,
          endDateTime: (selected == DrawerSelection.event) ? endDateTime?.toIso8601String() : null,
          isAllDay: (selected == DrawerSelection.event) ? isAllDay : null,
          calendar: (selected == DrawerSelection.event) ? selectedCalendar : null,
          recurrenceRule: rRule,
        );
      }
  ...
  Widget build(BuildContext context) {
    ...
    final drawerWidth = screenWidth > 600 ? 420.0 : screenWidth * 0.85;
  ...
  Widget _buildCreateForm() {
    return Column(
      ...
      children: [
        ...
        const Divider(height: 1, thickness: 1, color: Colors.black12),
        Expanded(child: (selected == DrawerSelection.event) ? _eventFormUI() : _defaultFormUI()),
      ],
    );
  }
      ...
      child: Column(
        children: [
          _glassCard(child: Column(children: [
            ...
            if (selected == DrawerSelection.link) _input("URL", (v) => setState(() => newUrl = v), icon: Iconsax.link)
          ])),
          const SizedBox(height: 16),
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                child: pickedImage == null
                    ? Column(mainAxisAlignment: MainAxisAlignment.center, children: [const Icon(Iconsax.cloud_add, size: 40, color: Colors.blueAccent), Text("Upload Image", style: GoogleFonts.poppins(fontWeight: FontWeight.w500, color: Colors.blueAccent))])
                    : ClipRRect(borderRadius: BorderRadius.circular(15), child: Image.memory(pickedImage!, fit: BoxFit.cover)),
              ),
            ),
          ])),
          const SizedBox(height: 32),
  ...
  Widget _eventFormUI() {
    return LayoutBuilder(
      builder: (context, constraints) {
        final bool useVerticalInputs = constraints.maxWidth < 320;
        return SingleChildScrollView(
          padding: const EdgeInsets.all(20),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              ...
              _glassCard(child: Column(children: [
                _sectionHeader(Iconsax.calendar, "Date & Time"),
                const SizedBox(height: 16),
                if (useVerticalInputs) ...[
                  _modernPicker("Starts", startDate == null ? "Set Date" : "${startDate!.day}/${startDate!.month}/${startDate!.year}", Iconsax.calendar, () => _pickDate(true)),
                  const SizedBox(height: 12),
                  _modernPicker("At", startTime?.format(context) ?? "Set Time", Iconsax.clock, () => _pickTime(true)),
                ] else ...[
                  Row(children: [
                    Expanded(child: _modernPicker("Starts", startDate == null ? "Set Date" : "${startDate!.day}/${startDate!.month}/${startDate!.year}", Iconsax.calendar, () => _pickDate(true))),
                    const SizedBox(width: 12),
                ...
                const Divider(height: 32, color: Colors.black12),
                if (useVerticalInputs) ...[
                  _modernPicker("Ends", endDate == null ? "Set Date" : "${endDate!.day}/${endDate!.month}/${endDate!.year}", Iconsax.calendar, () => _pickDate(false)),
                  const SizedBox(height: 12),
                  _modernPicker("At", endTime?.format(context) ?? "Set Time", Iconsax.clock, () => _pickTime(false)),
                ] else ...[
                  Row(children: [
                    Expanded(child: _modernPicker("Ends", endDate == null ? "Set Date" : "${endDate!.day}/${endDate!.month}/${endDate!.year}", Iconsax.calendar, () => _pickDate(false))),
                    const SizedBox(width: 12),
                ...
                SwitchListTile.adaptive(contentPadding: EdgeInsets.zero, title: Text("All Day", style: GoogleFonts.poppins(fontSize: 14)), value: isAllDay, onChanged: (v) => setState(() => isAllDay = v)),
              ])),
              const SizedBox(height: 16),
              _buildRecurrenceSection(),
              const SizedBox(height: 16),
              _glassCard(child: Column(children: [
                ...
                const SizedBox(height: 12),
                _modernCalendarPicker(value: selectedCalendar, items: ['Church Calendar', 'Personal', 'Work'], onChanged: (v) => setState(() => selectedCalendar = v!))
              ])),
              const SizedBox(height: 32),
              _actionButtons(),
            ],
  ...
  Widget _buildRecurrenceSection() {
    return _glassCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Container(
        ...
        child: Row(children: RecurrenceType.values.map((type) {
          bool isSelected = recurrence == type;
          return Expanded(child: GestureDetector(
              onTap: () => setState(() => recurrence = type),
              child: AnimatedContainer(
                duration: const Duration(milliseconds: 200), padding: const EdgeInsets.symmetric(vertical: 8),
                decoration: BoxDecoration(color: isSelected ? Colors.white : Colors.transparent, borderRadius: BorderRadius.circular(8), boxShadow: isSelected ? [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 4, offset: const Offset(0, 2))] : []),
                child: Center(child: Text(type == RecurrenceType.none ? "Off" : type.name.capitalize(), style: GoogleFonts.poppins(fontSize: 12, fontWeight: isSelected ? FontWeight.w600 : FontWeight.w400, color: isSelected ? Colors.blueAccent : Colors.grey.shade600))),
              )),
          );
        }).toList()),
      ),
      if (recurrence == RecurrenceType.weekly) ...[const SizedBox(height: 24), _buildDaySelector()],
      const SizedBox(height: 24),
      _buildMonthGrid(),
      const SizedBox(height: 24),
      _modernPicker("Recurrence Ends", recurrenceUntil == null ? "Repeat Forever" : "${recurrenceUntil!.day}/${recurrenceUntil!.month}/${recurrenceUntil!.year}", Iconsax.calendar_edit, () async {
  ...
  Widget _buildDaySelector() {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text("Repeat on", style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.w500, color: Colors.grey)),
      const SizedBox(height: 12),
      Wrap(alignment: WrapAlignment.spaceBetween, spacing: 4, runSpacing: 8, children: List.generate(7, (index) {
        final dayNum = index + 1; bool isSelected = selectedDaysOfWeek.contains(dayNum);
        return GestureDetector(
          onTap: () => setState(() => isSelected ? selectedDaysOfWeek.remove(dayNum) : selectedDaysOfWeek.add(dayNum)),
          child: AnimatedContainer(
            duration: const Duration(milliseconds: 200), width: 38, height: 38,
            decoration: BoxDecoration(color: isSelected ? Colors.blueAccent : Colors.white, shape: BoxShape.circle, border: Border.all(color: isSelected ? Colors.blueAccent : Colors.grey.shade200)),
            child: Center(child: Text(days[index], style: GoogleFonts.poppins(fontSize: 13, fontWeight: FontWeight.w600, color: isSelected ? Colors.white : Colors.grey.shade700))),
          ),
        );
      })),
    ]);
  ...
  Widget _buildMonthGrid() {
    return LayoutBuilder(builder: (context, constraints) {
      int crossAxisCount = constraints.maxWidth > 350 ? 4 : 3;
      final months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
      return GridView.builder(
        shrinkWrap: true, physics: const NeverScrollableScrollPhysics(),
        gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: crossAxisCount, crossAxisSpacing: 8, mainAxisSpacing: 8, childAspectRatio: 2.2),
        itemCount: 12,
        itemBuilder: (context, index) {
          final monthNum = index + 1; bool isSelected = selectedMonths.contains(monthNum);
          return GestureDetector(
            onTap: () => setState(() => isSelected ? selectedMonths.remove(monthNum) : selectedMonths.add(monthNum)),
            child: AnimatedContainer(
              duration: const Duration(milliseconds: 200), decoration: BoxDecoration(color: isSelected ? Colors.blueAccent.withOpacity(0.1) : Colors.grey.shade50, borderRadius: BorderRadius.circular(8), border: Border.all(color: isSelected ? Colors.blueAccent : Colors.transparent)),
              child: Center(child: Text(months[index], style: GoogleFonts.poppins(fontSize: 11, fontWeight: isSelected ? FontWeight.w600 : FontWeight.w400, color: isSelected ? Colors.blueAccent : Colors.grey.shade600))),
            ),
          );
        },
      );
  ...
  // --- HELPER COMPONENTS ---
  Widget _glassCard({required Widget child}) => Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.04), blurRadius: 20, offset: const Offset(0, 10))]), child: child);
  Widget _sectionHeader(IconData icon, String title) => Row(children: [Icon(icon, size: 20, color: Colors.blueAccent), const SizedBox(width: 8), Text(title, style: GoogleFonts.poppins(fontWeight: FontWeight.w600, fontSize: 15, color: Colors.blueGrey.shade700))]);
  Widget _input(String hint, Function(String) onChanged, {IconData? icon}) => Container(margin: const EdgeInsets.only(bottom: 12), decoration: BoxDecoration(color: Colors.grey.shade50, borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.grey.shade200)), child: TextField(onChanged: onChanged, decoration: InputDecoration(hintText: hint, prefixIcon: Icon(icon, size: 20, color: Colors.grey), border: InputBorder.none, contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12))));
  Widget _modernPicker(String label, String value, IconData icon, VoidCallback onTap) => InkWell(onTap: onTap, borderRadius: BorderRadius.circular(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(label, style: GoogleFonts.poppins(fontSize: 11, color: Colors.grey.shade600, fontWeight: FontWeight.w500)), const SizedBox(height: 4), Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.blueAccent.withOpacity(0.05), borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.blueAccent.withOpacity(0.1))), child: Row(children: [Icon(icon, size: 16, color: Colors.blueAccent), const SizedBox(width: 8), Expanded(child: FittedBox(fit: BoxFit.scaleDown, alignment: Alignment.centerLeft, child: Text(value, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600))))]))]));
  Widget _modernCalendarPicker({required String value, required List<String> items, required Function(String?) onChanged}) => Container(padding: const EdgeInsets.symmetric(horizontal: 12), decoration: BoxDecoration(color: Colors.grey.shade50, borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.grey.shade200)), child: DropdownButtonHideUnderline(child: DropdownButton<String>(isExpanded: true, value: value, items: items.map((e) => DropdownMenuItem(value: e, child: Text(e, style: GoogleFonts.poppins(fontSize: 14)))).toList(), onChanged: onChanged)));
  Widget _actionButtons() => Column(children: [SizedBox(width: double.infinity, height: 55, child: ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.blueAccent, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)), elevation: 0), onPressed: isSaving || newTitle.trim().isEmpty ? null : _handleCreateNewList, child: isSaving ? const CircularProgressIndicator(color: Colors.white) : Text("Create ${selected.name.capitalize()}", style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)))), TextButton(onPressed: _resetState, child: Text("Discard", style: TextStyle(color: Colors.red.shade400)))]);
  Widget _buildListImage(String imageUrl) => Container(width: 45, height: 45, decoration: BoxDecoration(color: Colors.grey.shade50, borderRadius: BorderRadius.circular(10)), child: imageUrl.isNotEmpty ? ClipRRect(borderRadius: BorderRadius.circular(10), child: Image.network(imageUrl, fit: BoxFit.cover, errorBuilder: (_, __, ___) => const Icon(Iconsax.image))) : const Icon(Iconsax.image, color: Colors.grey));
  Widget _buildEmptyState() => Center(child: Column(children: [const SizedBox(height: 40), Icon(Iconsax.ghost, size: 40, color: Colors.grey.shade300), const SizedBox(height: 8), Text("No results found", style: GoogleFonts.poppins(color: Colors.grey, fontSize: 13))]));
...
class FallingListItem extends StatefulWidget {
  ...
  const FallingListItem({super.key, required this.child, required this.delay});
  @override State<FallingListItem> createState() => _FallingListItemState();
}

class _FallingListItemState extends State<FallingListItem> with SingleTickerProviderStateMixin {
  bool _visible = false;
  @override void initState() { super.initState(); Future.delayed(widget.delay, () { if (mounted) setState(() => _visible = true); }); }
  @override Widget build(BuildContext context) { return AnimatedSlide(offset: _visible ? Offset.zero : const Offset(0, -0.3), duration: const Duration(milliseconds: 500), curve: Curves.easeOutBack, child: AnimatedOpacity(opacity: _visible ? 1.0 : 0.0, duration: const Duration(milliseconds: 500), child: widget.child)); }
}