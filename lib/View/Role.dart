...
class _RolesPageState extends State<RolesPage> with TickerProviderStateMixin {
        ...
        return LayoutBuilder(builder: (context, constraints) {
          ...
          return Scaffold(
            ...
            appBar: AppBar(
              ...
              actions: [
                _buildHeaderButton(
                  label: isMobile ? "Roles" : "Manage Roles",
                  icon: Icons.settings_suggest_outlined,
                  onTap: () => openManageRolesSheet(context),
                ),
                const SizedBox(width: 8),
                ...
                SliverToBoxAdapter(
                  child: Padding(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        _buildTopControls(controller, isMobile),
                        const SizedBox(height: 32),
                        _buildPermissionHeader(isMobile),
                      ],
                    ),
                  ),
                ),
                SliverPadding(
                  ...
                  sliver: SliverList(
                    delegate: SliverChildBuilderDelegate(
                          (context, index) {
                        String category = controller.sidebarCategories.keys.elementAt(index);
                        return _buildCategorySection(category, controller, isMobile);
                      },
                      childCount: controller.sidebarCategories.length,
                    ),
                  ),
            ...
            floatingActionButtonLocation: FloatingActionButtonLocation.centerFloat,
            floatingActionButton: _buildSaveButton(controller, isMobile),
          );
        });
      },
    );
  ...
  Widget _buildTopControls(RolesController controller, bool isMobile) {
    return Container(
      ...
      child: Column(
        children: [
          // Adaptive Row/Column for Role Selection
          if (isMobile) ...[
            ...
            Row(
              ...
              children: [
                Expanded(flex: 2, child: _buildRoleDropdown(controller)),
                const SizedBox(width: 20),
                Expanded(
                  child: _buildActionButton(
                    label: "New Role",
          ...
          if (showNewRoleFields) _buildNewRoleForm(controller, isMobile),
        ],
      ),
    );
  }

  Widget _buildRoleDropdown(RolesController controller) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text("Select Active Role", style: GoogleFonts.poppins(fontSize: 14, color: Colors.grey[600])),
  ...
  Widget _buildNewRoleForm(RolesController controller, bool isMobile) {
    return Padding(
      padding: const EdgeInsets.only(top: 20),
      child: Column(
        children: [
          if (isMobile) ...[
            _buildTextField(roleController, "Role Name", Icons.badge_outlined),
            const SizedBox(height: 12),
            _buildTextField(roleDescController, "Description", Icons.description_outlined),
          ] else
            Row(
              children: [
                Expanded(child: _buildTextField(roleController, "Role Name", Icons.badge_outlined)),
          ...
          Align(
            ...
            child: SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: Text("Confirm Creation", style: GoogleFonts.poppins(color: Colors.white, fontWeight: FontWeight.bold)),
              ),
            ),
          )
        ],
  ...
  Widget _buildCategorySection(String category, RolesController controller, bool isMobile) {
    List<Map<String, dynamic>> items = controller.sidebarCategories[category]!;
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Padding(
          ...
          child: Row(
            children: [
              Container(width: 4, height: 20, decoration: BoxDecoration(color: accentCyan, borderRadius: BorderRadius.circular(2))),
              const SizedBox(width: 8),
              Text(category.toUpperCase(), style: GoogleFonts.poppins(fontWeight: FontWeight.bold, letterSpacing: 1.2, color: Colors.blueGrey, fontSize: isMobile ? 12 : 14)),
            ],
          ),
        ),
        Container(
              ...
              return Padding(
                ...
                child: Row(
                  children: [
                    ...
                    _buildPermissionToggle(controller.sidebarPermissionStates[key]?['view'] ?? false, (v) => controller.toggleSidebarPermission(key, 'view'), isMobile),
                    _buildPermissionToggle(controller.sidebarPermissionStates[key]?['read'] ?? false, (v) => controller.toggleSidebarPermission(key, 'read'), isMobile),
                    _buildPermissionToggle(controller.sidebarPermissionStates[key]?['manage'] ?? false, (v) => controller.toggleSidebarPermission(key, 'manage'), isMobile),
                    _buildPermissionToggle(controller.getSidebarAll(key), (v) => controller.toggleSidebarAll(key, v), isMobile, isAll: true),
                  ],
                ),
              );
            },
  ...
  Widget _buildPermissionToggle(bool value, Function(bool) onChanged, bool isMobile, {bool isAll = false}) {
    return Expanded(
      child: Center(
        child: InkWell(
          onTap: () => onChanged(!value),
  ...
  Widget _buildSaveButton(RolesController controller, bool isMobile) {
    return InkWell(
      onTap: () async {
        if (selectedRoleId == null) return;
        setState(() => isSaving = true);
  ...
  Widget _buildHeaderButton({required String label, required IconData icon, required VoidCallback onTap}) {
    return TextButton.icon(
      onPressed: onTap,
      icon: Icon(icon, color: primaryTeal, size: 18),
      label: Text(label, style: GoogleFonts.poppins(color: primaryTeal, fontWeight: FontWeight.w500, fontSize: 13)),
    );
  }
  ...
  Widget _buildPermissionHeader(bool isMobile) {
    return Row(
      children: [
        Expanded(flex: isMobile ? 2 : 3, child: const SizedBox()),
        _headerLabel("VIEW", isMobile),
        _headerLabel("READ", isMobile),
        _headerLabel("MANAGE", isMobile),
        _headerLabel("ALL", isMobile),
      ],
    );
  }

  Widget _headerLabel(String label, bool isMobile) {
    return Expanded(
      child: Center(
        child: Text(
            label,
