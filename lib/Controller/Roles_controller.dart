...
class RolesController extends ChangeNotifier {
  ...
  Future<void> fetchRoles() async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        debugPrint("❌ Failed to fetch roles: ${res.statusCode}");
      }
    } catch (e) {
      debugPrint("⚠️ fetchRoles error: $e");
    }
    isLoading = false;
    notifyListeners();
  }
  ...
  Future<void> fetchPermissions(String roleId) async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        debugPrint("❌ Failed to fetch permissions for $roleId");
      }
    } catch (e) {
      debugPrint("⚠️ fetchPermissions error: $e");
    }
    isLoading = false;
    notifyListeners();
  }
  ...
  Future<void> fetchSidebarForRole(String roleId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint("❌ Failed to fetch sidebar for $roleId");
      }
    } catch (e) {
      debugPrint("⚠️ fetchSidebarForRole error: $e");
    } finally {
      isLoading = false;
      notifyListeners();
    }
  ...
  /// 🔹 Get only the marked sidebar items
  List<Map<String, dynamic>> getMarkedSidebarItems() {
    return sidebarPermissionStates.entries
        .where((entry) => entry.value.values.any((v) => v == true))
        .map((entry) {
      return {
        "key": entry.key,
        "permissions": {
  ...
  Future<void> createRole({
    ...
    try {
      ...
      debugPrint('📡 CREATE ROLE → ${response.statusCode}');
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        roles.insert(0, newRole); // add to top
        notifyListeners();
      } else {
        throw Exception(
          'Failed to create role: ${response.statusCode}, ${response.body}',
      ...
      debugPrint('⚠️ Error creating role: $e');
      rethrow;
    }
  }
  ...
  Future<void> deleteRole(String roleId) async {
    ...
    try {
      ...
      debugPrint("📡 deleteRole($roleId) → ${res.statusCode}");

      if (res.statusCode == 200) {
        roles.removeWhere((role) => role['_id'] == roleId);
        notifyListeners();
        debugPrint("✅ Role $roleId deleted");
      } else {
        debugPrint("❌ Failed to delete role: ${res.statusCode} :: ${res.body}");
      }
    } catch (e) {
      debugPrint("⚠️ deleteRole error: $e");
      rethrow;
    }
  }
}
